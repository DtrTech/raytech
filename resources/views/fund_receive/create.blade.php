                ...
                <div class="d-flex breadcrumb-content">
                    <div class="page-header">
                        ...
                        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item">Fund Receive</li>
        ...
        <div id="basic" class="col-lg-6 layout-spacing">
            <div class="statbox widget box box-shadow">
                ...
                <div class="widget-content widget-content-area" style="padding: 16px 15px">
                    ...
                    <div class="row" >
                        <form enctype="multipart/form-data" @if (isset($fund_receive)) method="post" action="{{ route('fund_receive.update',$fund_receive) }}" @else method="post" action="{{ route('fund_receive.store') }}" @endif>
                        @csrf
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-select" aria-label="Default select example" name="purchase_order_id" id="purchase_order_id">
                                    ...
                                    @foreach($purchase_order as $g)
                                        <option value="{{ $g->id }}" {{ isset($fund_receive) && $fund_receive->purchase_order_id == $g->id ? 'selected' : '' }}>{{ $g->po_no ?? "" }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-12 col-12">
                            <div class="form-group">
                                ...
                                <select class="form-select" aria-label="Default select example" name="user_id" id="user_id">
                                    <option value="">-- Select --</option>
                                    @foreach($purchase_order as $g)
                                        <option value="{{ $g->user_id }}" data-po-id="{{ $g->id }}" style="display: none" {{ isset($fund_receive) && $fund_receive->user_id == $g->user_id ? 'selected' : '' }}>{{ $g->user->name ?? "" }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-lg-12 col-12 ">
                            <div class="form-group">
                                ...
                                <input id="t-text" type="text" name="amount" placeholder="amount" class="form-control" value="{{$fund_receive->amount??''}}" required>
                            </div>
                        </div>
                        <div class="col-lg-12 col-12 ">
                            <div class="form-group">
                                <label for="t-text">Remark</label>
                                <input id="t-text" type="text" name="remark" placeholder="remark" class="form-control" value="{{$fund_receive->remark??''}}" required>
                            </div>
                        </div> 
                        <div class="col-lg-12 col-12">
                            <div class="form-group">
                                ...
                                <select class="form-select" aria-label="Default select example" name="currency" id="currency">
                                    <option value="">-- Select --</option>
                                    @foreach($purchase_order as $g)
                                        <option value="{{ $g->currency_from_id }}" data-po-id="{{ $g->id }}" style="display: none" {{ isset($fund_receive) && $fund_receive->currency_from_id == $g->currency_from_id ? 'selected' : '' }}>{{ $g->currency_from->short_name ?? "" }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <button type="submit" class="mt-2  btn btn-primary float-right" style="margin-right:10px">Save</button>
                        <a href="{{route('fund_receive.index')}}" class="mt-2  btn btn-warning float-right" style="margin-right:10px">Back</a>
                        </form> 
                    </div>
                </div> 
            </div>
